...
![Importar dados](images/import.png)
Pode importar um dataset no RStudio através de cliques no painel "Files" é um processo intuitivo , especialmente para quem prefere uma abordagem mais visual em vez de escrever código manualmente. Os passos são os seguintes:
...
3. **Localizar o dataset**: Navegue até a pasta onde o seu arquivo de dados está armazenado. Se você acabou de iniciar um projeto novo, pode ser necessário primeiro copiar ou mover o arquivo de dados para a pasta do projeto.
...
5. **Executar e Salvar o Código de Importação**:
   ...
   - **Têm copiar e colar esse código para o script R para utilizar e fazer render.** 
...
Ao trabalhar com R, nomear clara e consistente as colunas é crucial para garantir a facilidade de leitura, compreensão e manipulação dos dados. Isso é especialmente importante quando os nomes das colunas servem como referência para legendas de figuras. Abaixo, encontraram-se as boas práticas para nomear as colunas em R:
...
**2. Evitar espaços**: Não usar espaços nos nomes das colunas. Espaços podem levar a erros de sintaxe e complicam o acesso aos dados. Utilize underscore (`_`) ou camelCase para separar palavras (Exemplo: `idadeMeses` ou `idade_meses`).
...
**4. Evitar caracteres especiais**: Não incluir caracteres como `&`, `#`, `<`, `>`, pois podem ser confundidos como operadores /(Rever módulo 1)

**5. Consistência entre colunas**: Mantenha um padrão em atribuir nomes a colunas semelhantes. Por exemplo, se usar prefixos ou sufixos em nomes de colunas relacionadas com datas, aplicar o *standard* de maneira consistente em todo o conjunto de dados (por exemplo, `dataInicio`, `dataFim`).
...
O mutate() é uma função usada para transformar ou adicionar novas colunas a um conjunto de dados (data frame ou tibble).  
...
Passámos de `r original_n` doentes para `r final_n` doentes para o follow-up.
...
Uso com group_by(): Frequentemente, summarise() é usada em conjunto com group_by(), o que permite calcular resumos agrupados por uma ou mais variáveis. Por exemplo, calcular a média de uma variável para diferentes grupos.

![Summarise](images/summarize.png)
...
Globalmente são objetos datetime mas podem ser POSIXt, POSIXct, and/or POSIXlt classes.

Existem alguns pacotes relevantes para lidade com dados em formato de data, mas o mais relevante é o "lubridate".

![Formato de datas no R](images/date_formats.jpeg) 
...
:::{.callout-warning}
Por vezes os progrmas guardam datas em formato numérico e é necessário informar o R sobre a origem, ou o valor 0 dos dados.
...
if_else(condição, verdadeiro, falso, tipo de dados em falta = NULL)
...
tipo de dados em falta: O tipo de dado a retornar em caso de valores NA. Se não especificado, assume-se NULL.
...
- Combina linhas de `a` e `b` que têm valores correspondentes na coluna especificada em `b`.
...
1. **Especificação da key**: As operações de join dependem da especificação de uma ou mais chaves-primárias (`by = "key"`). Esta key são colunas nas tabelas `a` e `b` que você deseja usar para combinar as linhas.
...
![Joins_by Inequalities Variants](images/image-2000523169.png){fig-align="center"}
...
**1. complete:** Função utilizada para completar dados em falta
  - Uso: Útil em dados em série temporal
  
**2. Funções genéricas de transformação:** (as.numeric, as.double, as.character, as.factor, as.Date)
  - Uso: tranformação entre classes de variáveis
  
**3. across** Manipulação de múltplas variáveis ao mesmo tempo
  - Uso: Usar a mesma função em várias colunas ao mesmo tempo
  - Sintaxe: summarise(dataframe, across(selection, function without parenthesis))
  
**4. Bind rows e Bind Colums:** Combinar datasets em relação a colunas iguais ou id iguais
  - Uso: Juntar dados 
  
:::{.callout-warning}

No bind rows temos de ter colunas com nomes em comum.

No bind cols temos de ter o mesmo número de linhas.
