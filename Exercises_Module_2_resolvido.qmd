...
Estes execícios são relativos ao módulo 2.
...
#| code-fold: true
#| echo: false
if(exists("hypoxia") & class(hypoxia)=="data.frame") {
...
if(exists("hypoxia_clean")& class(hypoxia)=="data.frame") {
  print(":) DataFrame 'hypoxia' criado com sucesso")
} else {
  print(":( DataFrame 'hypoxia' não existe")
...
if(exists("hypoxia_adult")& class(hypoxia_adult)=="data.frame") {
...
if(exists("hypoxia_adult2")& class(hypoxia_adult)=="data.frame") {
...
if(exists("hypoxia_final")& class(hypoxia_adult)=="data.frame") {
...
get_results(report)
save_report(report, output_file ="validation_report2.html")
...
```{=html}
<iframe width="100%" height=1000 src="validation_report2.html"></iframe>
```
