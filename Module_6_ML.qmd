...
A aprendizagem automática (Machine Learning) refere-se ao
desenvolvimento de algoritmos que permitem aos computadores aprenderem a
partir de dados, sem serem explicitamente programados para realizar uma
tarefa específica.

Estes algoritmos ajustam-se a padrões nos dados para fazer previsões ou
tomar decisões com base em novos dados. A aprendizagem automática é
utilizada em várias aplicações, como reconhecimento de voz, diagnósticos
médicos, recomendação de produtos, entre outros.

#### Aprendizagem Supervisionada

Na aprendizagem supervisionada, o algoritmo é treinado com um conjunto
de dados rotulado, ou seja, para cada entrada, existe uma saída esperada
(label). O objetivo é que o modelo consiga aprender a relação entre as
variáveis de entrada (features) e a variável de saída (target), de modo
a fazer previsões corretas para novos dados.

Problemas de Regressão

Em problemas de regressão, a variável alvo (target) é contínua, como
prever preços de casas, salários ou temperatura. O objetivo do modelo é
...
Nos problemas de classificação, a variável alvo é discreta, como
categorizar emails como “spam” ou “não spam”, ou classificar um paciente
como tendo ou não uma doença. O objetivo do modelo é prever a classe
correta para novas observações.
...
A aprendizagem não-supervisionada trabalha com dados não rotulados. O
objetivo é descobrir padrões ou estruturas escondidas nos dados sem ter
uma saída pré-definida. Técnicas comuns incluem:
...
-   Josh Starmer. (2024, Junho 15). StatQuest \[Videos\]. YouTube.
    https://www.youtube.com/@statquest/videos . Estatística e
    aprendizagem automática apresentados de forma simples, clara e
    rigorosa.
...
algoritmos aprendem (ou treinam) uma função $f$ com o dataset de treino,
que tem dados de input ($x$) e de output ($y$), para depois fazerem
previsões (com novos dados de input $x$).
...
Quanto mais treinam, melhor desempenham a tarefa de fazer previsões. O
treino faz-se com técnicas de validação cruzada.
...
Entre as funções e algoritmos usados em aprendizagem, nenhuma é melhor
que outras. Num dataset particular, um algoritmo pode funcionar melhor,
mas outro algoritmo pode ser melhor noutro dataset semelhante.
...
variância da função minimizam o valor de MSE\|ER -
...
O erro de treino, $\varepsilon$, é dado pela distância vertical entre
oponto e a linha.
...
superior porque terá um viés elevado.

Finalmente, o polinómio de grau 12 (C) apresenta o MSE mais elevado
porque terá maior variância.
...
em sobre-ajustamento (overfitting). Nos dados de treino o MSE é cada vez
mais baixo.
...
($n=`r nabs[2]`$, `r round(nrel[2]*100)`%) e benignos
($n=`r nabs[1]`$,`r round(nrel[1]*100)`%).
...
de teste (ER = `r err_knn1`). Vamos afinar classificador com $k=11$ e
...
observações para classificar cada ponto da região.

Nos dados de teste projectados nestas regiões vêem-se algumas
observações mal classificadas. A estimativa da taxa de erro de teste
está ilustrada no gráfico seguinte. Os classificadores mais e menos
flexiveis ($k= `r knn1k`$ e $k=`r knn5k`$, respectivamente) apresentam
uma taxa mais elevada em relação ao classificador com $k$ "intermédio"
...
procedimentos habituais para estimar o erro designam-se por validação
...
não-supervisionada[^module_6_ml-8]
...
-   *análise em componentes principais (ACP)*, usada para visualização
    de dados ou pre-processamento em aprendizagem supervisionada.
...
A análise exploratória (EDA) é essencial ao processo de modelação
estatística. Vamos ver exemplos de como esta etapa nos fornece
informação relevante para termos depois melhor desempenho no processo de
modelação.
...
`recipe()` e as funções `step_normalize()` e `step_dummy()`.

O argumento da função inclui a formula do modelo. O simbolo `~`
especifica que é uma formula, onde à sua esquerda está (o nome da)
variável resposta e à direita o dos preditores. O simbolo `.` indica que
pretendemos todos os preditores do dataset.
...
1)  estimar os parâmetros dos dados de treino necessários para serem
    depois aplicados a outros datasets e
...
Neste exemplo, vamos usar a regressão logística para prever se um
paciente tem (classe 1) ou não (classe 0) uma doença cardíaca.
...
O logit é uma função ligação que transforma a regressão logística num
modelo linear:
...
Num problema de classificação onde há apenas 2 classes, é usual prever
classe 1 se $\Pr(y=1|x)\ge 0.5$ e na classe 0 em caso contrário. Mas
podem escolher-se outras regras de decisão.
...
No `tidymodels` o termo *pipe* refere-se ao operador `%>%`, por isso,
para evitar ambiguidades, no `tidymodels` essa série designa-se por
*workflow*.
:::
...
[^module_6_ml-10]: as colunas novas começam com o prefixo `.` para
...
Embora tenhamos visto intuitivamente como responder, falta saber como é
que o algortimo o faz de forma automática.
...
por: $$
\text{g}(s, v) = E(s) - E(s, v) $$ Quanto maior o ganho, mais puro fica
o nó.
:::
...
*gananciosa* (greedy) criando em cada nó a partição que melhor resultado
dá (de acordo com o indice de Gini ou a entropia e ganho).

Este processo de construção designa-se por separação recursiva binária
(*recursive binary splitting*). Repetimos o procedimento até um critério
de paragem ser atingido. Podemos impôr limitações à construção da árvore
como por ex. parar quando tivermos no máximo 5 exemplos em cada nó-filho
(*pre-prunning*).
...
Da teoria estatística[^module_6_ml-11] sabemos que se tivermos um grande
número ($N$) datasets de treino independentes com variância $\sigma^2$,
a variância da média será $\sigma^2/N$.

[^module_6_ml-11]: Ver e.g. distribuição amostral da média, teorema do
    limite central

Sendo assim, em vez de podarmos as árvores para reduzir a variância,
podemos construir árvores compridas, complexas, que tendem a ter baixo
viés e usar métodos como o *bagging* para reduzir a variância do modelo.
