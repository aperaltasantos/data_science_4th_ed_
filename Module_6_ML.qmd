...
A aprendizagem automática (Machine Learning) refere-se ao desenvolvimento de algoritmos que permitem aos computadores aprenderem a partir de dados, sem serem explicitamente programados para realizar uma tarefa específica. 

Estes algoritmos ajustam-se a padrões nos dados para fazer previsões ou tomar decisões com base em novos dados. A aprendizagem automática é utilizada em várias aplicações, como reconhecimento de voz, diagnósticos médicos, recomendação de produtos, entre outros.

### Aprendizagem Supervisionada

Na aprendizagem supervisionada, o algoritmo é treinado com um conjunto de dados rotulado, ou seja, para cada entrada, existe uma saída esperada (label). O objetivo é que o modelo consiga aprender a relação entre as variáveis de entrada (features) e a variável de saída (target), de modo a fazer previsões corretas para novos dados.

Problemas de Regressão

Em problemas de regressão, a variável alvo (target) é contínua, como prever preços de casas, salários ou temperatura. O objetivo do modelo é prever valores numéricos.

Exemplo:

	•	Prever o preço de uma casa com base em características como tamanho, localização, número de quartos, etc.

Problemas de Classificação

Nos problemas de classificação, a variável alvo é discreta, como categorizar emails como “spam” ou “não spam”, ou classificar um paciente como tendo ou não uma doença. O objetivo do modelo é prever a classe correta para novas observações.

Exemplo:

	•	Classificar se um email é spam ou não com base nas suas características.

### Aprendizagem Não-Supervisionada

A aprendizagem não-supervisionada trabalha com dados não rotulados. O objetivo é descobrir padrões ou estruturas escondidas nos dados sem ter uma saída pré-definida. Técnicas comuns incluem:

	•	Clustering (agrupamento): Agrupa observações semelhantes em clusters (grupos).
	•	Redução de Dimensionalidade: Simplifica o número de variáveis enquanto mantém o máximo de informação possível.

Exemplo:

	•	Agrupar clientes com base nos seus padrões de compra.

### Bibliografia

Vamos usar packages *open-source*. 

Há vários bons recursos sobre os tópicos da aula. Sugiro estes:

-   Gareth James et al (2023) "An Introduction to Statistical Learning with Applications in R". Springer. Disponivel online:
    https://www.statlearning.com/ . Bem explicado, muitos exemplos R. Algum (pouco) formalismo estatistico.

-   Josh Starmer. (2024, Junho 15). StatQuest \[Videos\]. YouTube.
    https://www.youtube.com/@statquest/videos . Estatística e aprendizagem automática apresentados de forma simples, clara e rigorosa.
...
Esquema com a visão geral da aprendizagem supervisionada[^module_6_ml-1]

[^module_6_ml-1]: Adapt. de Kilian Weinberger 2017

```{r visao-learn, echo=FALSE, fig.align='left', fig.cap="Visão tradicional vs aprendizagem supervisionada", out.width = '50%'}
knitr::include_graphics("images/overview2.png")
```

O processo de aprendizagem supervisionada evolui com o treino. Os
algoritmos aprendem (ou treinam) uma função $f$ com o dataset de treino, que tem dados de input ($x$) e de output ($y$), para depois fazerem previsões (com novos dados de input $x$).

```{r fxy, echo=FALSE, eval = F, fig.align = "left", fig.cap="Algoritmo como função que estabelece a ligação entre x e y", out.width = '30%'}
...
Quanto mais treinam, melhor desempenham a tarefa de fazer previsões. O treino faz-se com técnicas de validação cruzada.
...
*features*, *input features*) e a pressão arterial é a resposta $y$ (ou *outcome*, *output*). Poderiamos experimentar treinar uma regressão linear ($f$) para mapear a relação entre os preditores e a resposta.
...
Entre as funções e algoritmos usados em aprendizagem, nenhuma é
melhor que outras. Num dataset particular, um algoritmo pode
funcionar melhor, mas outro algoritmo pode ser melhor noutro dataset semelhante.
...
Vamos fingir por momentos que somos Deus, e sabemos que a verdadeira relação entre a idade e a intensidade da dor tem esta forma: $$f(x)=x^4+ \epsilon$$ 
...
Vemos que os pontos não se sobrepõem exatamente à linha (função). 
Isto acontece porque há outros factores que não controlamos e que interferem na relação (ruído).
...
O erro de treino, $\varepsilon$, é dado pela distância vertical entre oponto e a linha. 
...
Aqui, o MSE é de treino pois é quantificado nos dados de treino. No gráfico seguinte vemos o MSE de treino estimado para as três funções.
...
O polinómio A nunca conseguirá captar a forma curva da relação entre idade e intensidade da dor (*underfitted*). Tem um viés elevado. O polinómio C tem o viés menor, pois a aproximação aos pontos é a elevada (entre as 3 funções).
...
Consegue captar a informação mais importante, a forma curva da relação e representa um bom compromisso entre tem um viés e a variáncia (*bias-variance trade-off*). O polinómio de grau um (A) apresenta um MSE superior porque terá um viés elevado.

Finalmente, o polinómio de grau 12 (C) apresenta o MSE mais elevado porque terá maior variância.
...
No caso de problemas de classificação, lidamos com respostas ($y$) qualitativas. Uma medida de erro usual para os classificadores é a taxa de erro: 
...
Para prever se um tumor é maligno ou benigno ($y$) a partir do seu tamanho e da idade do paciente ($\mathbf{x}$) vamos usar um dataset ($n= `r n`$) e treinar um algoritmo de classificação simples (e surpreendentemente eficaz), o $k$-NN (k-vizinhos mais próximos).
...
O dataset tem uma distribuição 'equilibrada' entre tumores malignos ($n=`r nabs[2]`$, `r round(nrel[2]*100)`%) e benignos ($n=`r nabs[1]`$,`r round(nrel[1]*100)`%).
...
A estratégia do algoritmo passa por classificar um novo tumor na classe mais comum entre os $k$ vizinhos mais próximos. Como em todos os algoritmos de aprendizagem, vamos começar por dividir aleatoriamente o dataset em dados de treino ($n_{tr}= `r nrow(knn_treino)`$) e dados de teste ($n_{ts}=`r nrow(knn_teste)`$):
...
O classificador cometeu erros de classificação em `r f_knn1` observações de teste (ER = `r err_knn1`). Vamos afinar classificador com $k=11$ e $k=41$ e avaliar os erros com dados de teste.
...
Em relação a $1$-nn as regiões de decisão com $11$-nn e $41$-nn tendem a apresentar uma fronteira de decisão menos recortada, porque usam mais observações para classificar cada ponto da região. 

Nos dados de teste projectados nestas regiões vêem-se algumas observações mal classificadas. A estimativa da taxa de erro de teste está ilustrada no gráfico seguinte. Os classificadores mais e menos flexiveis ($k= `r knn1k`$ e $k=`r knn5k`$, respectivamente) apresentam uma taxa mais elevada em relação ao classificador com $k$ "intermédio" ($k=`r knn3k`$). (ilustrado no gráfico seguinte).
...
E como podemos treinar e avaliar o desempenho de diferentes modelos? Os procedimentos habituais para estimar o erro designam-se por validação simples (que já usámos nos exemplos anteriores) ou validação cruzada:
...
validação[^module_6_ml-2]](valset.png){width="80%"}

[^module_6_ml-2]: Fonte: James, Gareth et al. *An Introduction to
    Statistical Learning with Applications in R*. New York :Springer,
    2013

[^module_6_ml-3]: Fonte: James, Gareth et al. *An Introduction to
    Statistical Learning with Applications in R*. New York :Springer,
    2013
...
k-fold[^module_6_ml-4]](images/kfold.png){width="80%"}

[^module_6_ml-4]: Fonte: James, Gareth et al. *An Introduction to
    Statistical Learning with Applications in R*. New York :Springer,
    2013

[^module_6_ml-5]: Fonte: James, Gareth et al. *An Introduction to
    Statistical Learning with Applications in R*. New York :Springer,
    2013
...
![Esquema ilustrativo da técnica
LOOCV[^module_6_ml-6]](images/looc.png){width="80%"}

[^module_6_ml-6]: Fonte: James, Gareth et al. *An Introduction to
    Statistical Learning with Applications in R*. New York :Springer,
    2013

[^module_6_ml-7]: Fonte: James, Gareth et al. *An Introduction to
    Statistical Learning with Applications in R*. New York :Springer,
    2013
...
Esquema com a visão geral da aprendizagem não-supervisionada[^module_6_ml-8]

[^module_6_ml-8]: Adapt. de Andrew Ng 2017
...
Vejamos o seguinte exemplo: imaginemos que temos um dataset como o da figura seguinte, temos dados mas não nos dizem o que fazer com eles.
...
Embora possa parecer que estas análises complexas necessitem de muitas linhas de código para extrair as curvas principais ou para classificar todas as curvas, é possível escrever o algoritmo apenas com duas linhas de código. 

# CASOS DE ESTUDO

Vamos recorrer ao dataset `HeartDisease` (disponível na biblioteca `MLDataR`) para prever a existência de uma doença cardíaca a partir de dados sobre pressão sanguínea, frequência cardíaca máxima, história de angina de peito e outros preditores. 
Os exemplos são dados de pacientes, anonimizados, disponibilizados pela British Heart Foundation.
...
Vamos codificar as variáveis (preditores e resposta) com tipos de dados adequados. Por ex. `HearDisease` é uma variável do tipo numérico, vamos convertê-la numa variável categorica (no R designa-se por `factor`).
...
df$FastingBS  <- factor(df$FastingBS, labels = c("Normal","Diabetes"))
df$RestingECG  <- factor(df$RestingECG)
df$Angina <- factor(df$Angina)
df$HeartDisease <- factor(df$HeartDisease, labels = c("N", "Y"))
```
...
### Análise exploratória (Exploratory Data Analysis)

A análise exploratória (EDA) é essencial ao processo de modelação estatística. Vamos ver exemplos de como esta etapa nos fornece informação relevante para termos depois melhor desempenho no processo de modelação.
...
Aqui observamos que o peso do sexo feminino `F` entre pacientes com doença cardíaca é menor que o peso do sexo feminino entre pacientes sem doença cardíaca. Este gráfico sugere que há diferenças na ocorrência da doença cardiaca em função do sexo.
...
correlacionados com a resposta, dando boas indicações quanto à sua capacidade preditiva.
...
Antes do treino, os dados passam por um pré-processamento para 'limpar' e transformar os dados. Estas operações são geralmente muito úteis para preparar os dados e fornecer um dataset mais adequado ao processo de aprendizagem[^module_6_ml-9]. 

As necessidades de pré-processamento variam com os modelos usados.

[^module_6_ml-9]: https://www.tmwr.org/pre-proc-table

Neste exemplo vamos proceder à implementação de operações frequentemente necessárias e que serão implementadadas em todos os classificadores que vamo usar:
...
# em cada partição
particao <- initial_split(df, strata = HeartDisease)

# atribuir nome/objeto aos
# datasets de treino e de teste
dftr <- training(particao)
dftt  <- testing(particao)
```

Conforme vimos em @sec-validacao podemos criarmos partições do dataset de treino para estimar o erro de validação. Vamos usar a função `vfold_cv()` para criar as ditas partições e guardar o resultado num objeto para uso nas fases de treino.

```{r validacao-kfold, echo = T}
set.seed(4800000)

k_folds <- vfold_cv(dftr, v = 10, strata = HeartDisease)
```

Vamos também normalizar preditores numéricos e criar variáveis dummy para preditores categoricos. Estas operações são feitas com a função `recipe()` e as funções `step_normalize()` e `step_dummy()`. 

O argumento da função inclui a formula do modelo. O simbolo `~` especifica que é uma formula, onde à sua esquerda está (o nome da) variável resposta e à direita o dos preditores. O simbolo `.` indica que pretendemos todos os preditores do dataset.

```{r recipe-norm-dummy, echo = T}
receita <- 
  recipe(HeartDisease ~ ., data = dftr) %>% 
  step_normalize(all_numeric(), -all_outcomes()) %>% 
  step_dummy(all_nominal(), -all_outcomes())
```

Vejamos os resultados do pré-processamentocom as funções `prep()` e `bake()`. 

Estas são usadas respectivamente para 

1) estimar os parâmetros dos dados de treino necessários para serem depois aplicados a outros datasets e 

2) executar os cálculos aos novos dados.
...
Com o modelo de regressão logístico o objectivo é prever o valor de uma variável categórica binária (classe 0 ou 1) como uma função linear de preditores. 

Neste exemplo, vamos usar a regressão logística para prever
...
O logit é uma função ligação que transforma a regressão logística num modelo linear: 

$$
\log\bigg[\frac{\Pr(y=1|x)}{1-\Pr(y=1|x)}\bigg] = z
$$

O termo $\frac{\Pr(y=1|x)}{1-\Pr(y=1|x)}$ designa-se usualmente por chances (*odds*).
...
aplicada em problemas de classificação. Para classificar uma observação numa das classes, combinamos o modelo de regressão logistico com um critério ou regra de decisão.
:::
...
Num problema de classificação onde há apenas 2 classes,
é usual prever classe 1 se $\Pr(y=1|x)\ge 0.5$ e na classe 0 em caso contrário. Mas podem escolher-se outras regras de decisão.

### Treino

Agora que temos o pré-processamento terminado vamos preparar a aprendizagem. Vamos usar a estrutura `tidy` para especificar o modelo (*model*) logístico de regressão, com o package `glm` (*engine*) para fazer classificação (*mode*):
...
Vamos usar a função `workflow()`para integrar de forma encadeada a receita definida na @sec-preprocessamento e as especificações do modelo.

::: {.callout-note title="workflow"}
Em Python e outras linguagens designa-se uma série de operações
encadeadas por *pipeline*. 

No `tidymodels` o termo *pipe* refere-se ao operador `%>%`, por isso, para evitar ambiguidades, no `tidymodels` essa série designa-se por *workflow*.
:::
...
Qual será a classe prevista nos dados de treino? Podemos prever com a função `predict()`, que devolve uma coluna com a classe prevista.

```{r log-predict2, echo =T, results='hide'}
# predict()
predict(log_fit, new_data = dftr)
```

Usando a função `augment()` (da biblioteca `broom`), podemos obter informação adicional sobre os exemplos (e.g. valores
previstos)[^module_6_ml-10].

[^module_6_ml-10]: as colunas novas começam com o prefixo `.` para evitar escrever em cima de variveis originais com o mesmo nome

```{r log-predict3, echo =T}
# augment()
broom::augment(log_fit, new_data = dftt)
```
...
Com a função `relocate()` podemos mudar a ordem das colunas. Vamos usar a função para ver lado-a-lado as classes observadas e previstas para os dados de treino:

```{r log-augmentfit, echo = T}
augment(log_fit, new_data = dftr) %>%
...
Podemos usar a função `yardstick::metric_set()`para devolver estas as medidas de uma só vez:
...
No argumento da função `fit_samples()` vamos usar o objecto que criámos na sub-secção @sec-preprocessamento para criar treinar as partições do dataset de treino e estimar o erro de validação.
...
Os resultados apresentados nesta seção resultam do ajustamento aos dados de treino. Mas como vimos antes, o que pretendemos é usar este modelo com dados novos. Será que com novos dados este modelo será adequado?


Vamos usar os dados e o modelo especificado, mas agora com procedimentos que nos permitirão ter uma estimativa do erro generalizavel.
...
Na sub-secção @sec-classificacao vimos como funciona genericamente o algoritmo $k$-NN. Nesta sub-secção vamos implementar o algoritmo. Vamos introduzir no processo de treino uma afinação para o valor de $k$ (número de vizinhos mais próximos) para minimizar o erro (1-*accuracy*) de classificação.
...
Vamos usar o argumento `tune()` para "afinar" o valor do hiperparâmetro $k$ através de validação cruzada. Para tal temos de definir uma grelha (neste caso é apenas um vector).

```{r knn-tunegrid, echo = T}
k_grid <- tibble(neighbors = seq(1,51, by = 2))
```

Vamos juntar o modelo à receita especificada na @sec-preprocessamento, integrando tudo num workflow:
...
O último passo é usar a função `finalize_workflow()` para adicionar ao nosso workflow o modelo com o hiperparâmetro $k$ melhor afinado:
...
A função `last_fit()` ajusta o modelo final ao conjunto de dados de treino, e avalia o desempenho nos dados de teste. Em baixo, a função `collect_predictions()` permite visualizar num `tibble` os resultados da classificação com dados de teste
...
desenvolvidos na década de 1980, e podem ser aplicados em problemas de classificação ou de regressão. São simples de implementar e úteis para interpretar.
...
Vamos ver a intuição antes de vermos uma árvore. No gráfico seguinte temos os dados da @sec-classificacao. São dados de 120 pacientes que têm um tumor benigno (azul) ou tumor maligno (vermelho). Para cada paciente temos os valores de dois preditores: o tamanho do tumor e a idade do paciente.
...
ggplot(dftipo3, aes(x = tamanho, y = idade))+
  geom_point(aes(color=tumor), size = 2) + 
  geom_hline (yintercept = 44, color = "violet", size =1, linetype = "dashed") +
  scale_color_manual(name = "Tumor", values = c("blue", "red")) +
  xlab("Tamanho") + 
  ylab("Idade") +
  coord_fixed(ratio = .2)
...
ggplot(dftipo3, aes(x = tamanho, y = idade))+
  ...
  geom_segment(aes(x = 2.5 , y = 44, xend = 2.5, yend = 60),
               size =1, linetype = "dashed", color="violet") +
  scale_color_manual(name = "Tumor", values = c("blue", "red")) +
  xlab("Tamanho") + 
  ylab("Idade") +
  coord_fixed(ratio = .2)
...
1.  região do lado superior direito, com a grande parte dos tumores malignos, cujos pacientes são mais velhos, e têm tumores de maior dimensão;
2.  região do lado inferior, com larga maioria de tumores benignos detetados em pacientes mais jovens;
3.  região do lado superior esquerdo, onde estão a grande maioria de tumores benignos, observados em pacientes mais velhos.

A separação nestas três regiões permitiu segmentar o espaço dos
preditores em regiões, seguindo o racional das árvores de decisão. A figura seguinte ilustra a árvore de decisão obtida com os mesmos dados:
...
Estas partições especificadas pelo algoritmo correspondem à segmentação que fizemos 'a olho' para separar o espaço dos preditores em três regiões. 

Mas como o o algoritmo escolheu os preditores? 

E como sabe que valores usar para separar os dados (os exemplos)?

Embora tenhamos visto intuitivamente como responder, falta saber como é que o algortimo o faz de forma automática.
...
$$ onde $\hat{p}_{rk}$ é a proporção de exemplos na região $r$ que pertencem à classe $k$. Quanto maior a proporção de exemplos de uma só classe, menor o valor do índice nesse nó.
:::
...
$$ onde $\hat{p}_{rk}$ é a proporção de exemplos na região $r$ que pertencem à classe $k$.
...
O ganho de informação $g$ com nova partição usando o preditor $v$ é dado por: $$
\text{g}(s, v) = E(s) - E(s, v) $$ Quanto maior o ganho, mais puro fica o nó.
:::

Infelizmente, em termos computacionais é pesado percorrer todas as partições do espaço possiveis. Por essa razão, o processo de divisão de um nó é feito de cima para baixo (começa no *root-node*), de forma *gananciosa* (greedy) criando em cada nó a partição que melhor resultado dá (de acordo com o indice de Gini ou a entropia e ganho). 

Este processo de
construção designa-se por separação recursiva binária (*recursive binary splitting*). Repetimos o procedimento até um critério de paragem ser atingido. Podemos impôr limitações à construção da árvore como por ex. parar quando tivermos no máximo 5 exemplos em cada nó-filho (*pre-prunning*).
...
complexity prunning*) pode ser escolhido por validação cruzada (ou conjunto de validação).

::: {.callout-tip title="Custo de complexidade" collapse="true"}
Seja $R(T)$ uma medida global de impureza da árvore (e.g. indice de Gini ou Entropia) numa árvore com $T$ nós terminais. Pretende-se minimizar:
...
Vamos criar o *workflow* para juntar as tarefas de @sec-preprocessamento e as especificações para a árvore de decisão.
...
Vamos guardar num `tbl_df` (um dataframe com estrutura `tibble`) e atualizar o modelo especificando o valor do hiperparâmtero que devolveu maior exatidão.
...
significativamente melhorado usando métodos de *bagging*, *boosting* ou *random forests*. Na sub-secções seguintes vamos ver *bagging* e *random forests*.
...
1.  Gerar $n$ datasets independentes a partir do dataset de treino (*bootstrapping*);
2.  Treinar um modelo (árvore) para cada dataset gerado sem a preocupação de o podar;
3.  Usar cada modelo treinado para prever o valor de uma nova amostra;
4.  Prever a classe da nova amostra atribuindo a classe mais "votada" pelos modelos treinados
...
Da teoria estatística[^module_6_ml-11] sabemos que se tivermos um grande número ($N$) datasets de treino independentes com variância $\sigma^2$, a variância da média será $\sigma^2/N$. 

Sendo assim, em vez de podarmos as árvores para reduzir a variância, podemos construir árvores compridas, complexas, que tendem a ter baixo viés e usar métodos como o *bagging* para reduzir a variância do modelo.

[^module_6_ml-11]: Ver e.g. distribuição amostral da média, teorema do limite central

### Treino

Vamos implementar este algoritmo, recorrendo à biblioteca `baguette` que complementa a biblioteca `tidymodels` para implementação de *bagging*.
...
bag_fit <- bag_wflow %>% fit(dftr)
```

Dado que o processo de bagging envolve o ajustamento de muitas árvores, deixa de ser possível usar as árvores para interpretar resultados.

Podemos no entanto ver outras coisas. Na sub-secção @sec-arvores_intro vimos que podemos estimar a importância de um preditor no modelo com medidas de erro (e.g. o índice de Gini).
...
Vemos que os mais relevantes preditores para a ocorrência de doença cardiaca são `Cholesterol`, `MaxHR` e `HearPeakReading`.
...
# teste
bag_rs <- dftt %>%
  bind_cols(predict(bag_fit, dftt)) 
...
Apesar de usarmos *bootstap* para gerar datasets independentes, as árvores obtidas com *bagging* tendem a estar bem correlacionadas, porque todas usam os mesmos preditores. Imagine um caso em que um único preditor tem uma importância muito elevada em relação a todos os outros.

Com *bagging* a grande maioria (ou todas) das árvores iriam usar esse preditor no *root-node* e consequentemente as árvores seriam parecidas entre si.

Com *random forests*, procura-se "aliviar" esta correlação entre
modelos. Ao não usar todos os preditores em cada partição, as árvores são levadas a usar diferentes preditores a separar em diferentes alturas. Este procedimento reduz a correlação entre os modelos (árvores), tornando-os mais independentes entre si. Como usamos um número grande de árvores neste processo, e depois calculamos a média, este procedimento tende a melhorar o desempenho do modelo em relação ao ensemble de árvores obtido por *bagging*.
:::
