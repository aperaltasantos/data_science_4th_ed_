...
inpatient_hospital_lite <- inpatient_hospital_lite |>
  mutate(
    ...
    eriodo=as.Date(periodo, format = "%Y-%m-%d"),
    year = year(periodo),
...
- Existem alguns pacotes relevantes para lidade com dados em formato de data, mas o mais relevante será o "lubridate".
...
- Podem ser utilizadas no argumento format da função as.Date()
...
# INTERMIDIATE/ADVANCED DATA WRANGLING
...
Os fluxos if_else são úteis para ir definindo níveis para determinda condição 

:::{.callout-note title="Utilizadores de Excel"}
No Excel será semelhante à utilização da fórmula IF ou SE
:::
...
### Case_when - Opção Tidyverse para apresentação de condições if_else
...
Agora vamos criar um data wrangling mais complexo. 
Se a variável contiver uma expressão, podemos usar grepl para capturá-la e criar uma nova variável.
...
inpatient_hospital_wide <- inpatient_hospital_sum |>
  pivot_wider(
    names_from = tipo_instituicao, 
    values_from = doentes_saidos_var
    )
...
inpatient_hospital_long <- inpatient_hospital_wide |>
  pivot_longer(!periodo, 
               names_to = "tipo_instituicao", 
               values_to = "doentes_saidos_var")
...
```{=html}
<iframe width="780" height="500" src="https://data-lessons.github.io/gapminder-R/12-joins.html" title="Joins"></iframe>
```
...
## Extras de Exportação Específica
